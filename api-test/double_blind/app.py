...
with gr.Blocks(title="EasyPhoto", css="style.css") as app:
    ...
    with gr.Column(visible=False, elem_id="main"):
        ...
        with gr.Row():
            with gr.Column(scale=3):
                reference_imgs = gr.Gallery(
                    [], columns=3, rows=1, label="", show_label=True, elem_id="reference-imgs", visible=template["show_references"]
                )
        ...
        for question in template["questions"]:
            eval_results.append(gr.Radio(["Left", "Tie", "Right"], label=question, elem_classes="question"))

        submit = gr.Button(" Submit")
        next_btn = gr.Button("Change Another")
